...
sched_create_task(char *executable)
{
	...
	if (p < 0) {perror("fork"); exit(1);}
		...
		ins_proc(nproc-1, p, executable);		//add to process list
	}
}
...
sigchld_handler(int signum)
{
	...
	for(;;){
		...
		if (WIFEXITED(status)  || WIFSIGNALED(status)) {
			...
			del_proc(current_proc->id);				//delete process from process list
		}
		...
		printf("        The child process %d with id=%d is continuing.\n", current_proc->PID, current_proc->id);
...
int main(int argc, char *argv[]){
	...
	for (i=1; i < nproc; i++){
		...
		if (p < 0) {perror("fork"); exit(1);}
			...
			ins_proc(i, p, argv[i]); 		//add to process list
		}
	}	
